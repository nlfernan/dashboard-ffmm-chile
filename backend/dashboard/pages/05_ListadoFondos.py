...
def _alias(_df: pd.DataFrame, target: str, candidates, default=np.nan):
    """Si target no existe/baldío, crea a partir del primer candidato con datos."""
...
if "fecha_dia" not in df.columns or df["fecha_dia"].isna().all():
    ...
    for c in ["fecha_dia", "fecha_inf_date", "fecha", "fecha_inf"]:
        if c in df.columns:
            ...
            break
    if cand is not None:
        tmp = pd.to_datetime(df[cand], errors="coerce").dt.date
        df["fecha_dia"] = pd.to_datetime(tmp, errors="coerce")
    else:
        df["fecha_dia"] = pd.NaT
else:
...
# --------- Subset mínimo ----------
base = df[["run_fm", "nom_adm", "nombre_corto", "venta_neta_mm", "patrimonio_neto_mm", "fecha_dia"]].copy()

# ===============================
# 🧷 Nombre representativo: el de la ÚLTIMA FECHA (según fecha_dia)
# ===============================
tiene_fecha = base["fecha_dia"].notna().any()
if tiene_fecha:
    ultima_fecha = base["fecha_dia"].max()
    nombres_rep = (

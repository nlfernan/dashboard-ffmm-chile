...
# Etiquetas estándar del app.py (solo para UI)
SINDATO = "(Sin dato)"
...
with col1:
    fecha_sel = st.selectbox("Fecha", options=fechas_disponibles, index=0, key="afps_fecha")
...
with col2:
    ventana = st.radio("Ventana", options=ordenadas, horizontal=True, index=0, key="afps_ventana")

admins_univ = sorted(df.loc[df["fecha"] == fecha_sel, "administradora"].dropna().astype(str).unique().tolist())
with col3:
    admins_sel_raw = multiselect_con_todo("Administradora(s)", admins_univ, key="afps_admins")

aplicar = st.button("Aplicar filtros", type="primary", key="afps_aplicar")

# ===============================
# 🧮 Aplicación de filtros (aislado en session_state)
# ===============================
if "afps_df_filtrado" not in st.session_state or aplicar:
    ...
    st.session_state.afps_df_filtrado = df.loc[cond].copy()

df_filtrado = st.session_state.afps_df_filtrado
...
with tab_graf:
    ...
    labels = base.mark_text(align="left", dx=7, dy=-7, fontSize=10).encode(text="Fondo:N")
